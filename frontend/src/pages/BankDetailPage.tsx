    ...
    <div style={{ minHeight: '100vh', background: 'var(--app-bg-page)', padding: '24px' }}>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
          <button onClick={() => navigate(`/banks/${bankId}/wrong-answers`)}
                            ...
                            <div style={{
                              ...
                              <div onClick={() => setExpandedPoint(expandedPoint === point.id ? null : point.id)}
                                ...
                                {pointQuestions[point.id]?.length > 0 && (
                                  <span style={{ fontSize: '10px', color: 'var(--app-indigo)' }}>{pointQuestions[point.id].length} 题</span>
                                )}
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: 600, maxWidth: '90vw', maxHeight: '85vh', overflow: 'auto', padding: '28px', boxShadow: '0 8px 32px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center', marginTop: '12px' }}>
          ...
          {q.knowledge_point_uuids?.length > 0 && (
            <span style={{ fontSize: '11px', padding: '2px 10px', borderRadius: 6, background: 'rgba(99,102,241,0.08)', color: 'var(--app-indigo)' }}><LinkIcon size={11} color="#6366F1" /> 关联 {q.knowledge_point_uuids.length} 个知识点</span>
          )}
        </div>
      </div>
    </div>
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: 640, maxWidth: '95vw', maxHeight: '90vh', overflow: 'auto', padding: '28px', boxShadow: '0 8px 32px rgba(0,0,0,0.1)' }}>
        ...
        {!isEditing && (
          <div style={{ display: 'flex', gap: '4px', marginBottom: '20px', background: 'var(--app-bg-page)', borderRadius: 10, padding: '3px' }}>
            ...
            <button onClick={() => setTab('ai')} style={{ flex: 1, padding: '8px', borderRadius: 8, border: 'none', fontSize: '13px', cursor: 'pointer', background: tab === 'ai' ? '#fff' : 'transparent', color: tab === 'ai' ? 'var(--app-indigo)' : 'var(--app-text-secondary)', fontWeight: 500 }}><BotIcon size={14} color="#6366F1" /> AI 生成</button>
          </div>
        )}
                ...
                {ALL_Q_TYPES.map(t => {
                  ...
                  return (
                    <button key={t} onClick={() => setAiTypes(prev => prev.includes(t) ? prev.filter(x => x !== t) : [...prev, t])}
                      style={{ padding: '6px 16px', borderRadius: 20, border: '1.5px solid', cursor: 'pointer', fontSize: '13px', background: sel ? 'rgba(99,102,241,0.1)' : '#fff', color: sel ? 'var(--app-indigo)' : 'var(--app-text-secondary)', borderColor: sel ? '#818CF8' : 'var(--app-border)', fontWeight: sel ? 600 : 400 }}>
                      {QTYPE_LABELS[t]}
                    </button>
                  )
                })}
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                {['beginner', 'basic', 'intermediate', 'advanced', 'competition'].map(d => (
                  <button key={d} onClick={() => setAiDifficulty(d)}
                    style={{ padding: '6px 18px', borderRadius: 20, border: '1.5px solid', cursor: 'pointer', fontSize: '13px', background: aiDifficulty === d ? 'rgba(99,102,241,0.1)' : '#fff', color: aiDifficulty === d ? 'var(--app-indigo)' : 'var(--app-text-secondary)', borderColor: aiDifficulty === d ? '#818CF8' : 'var(--app-border)', fontWeight: aiDifficulty === d ? 600 : 400 }}>
                    {DIFF_LABELS[d] || d}
                  </button>
                ))}
              </div>
            ...
            {aiGenQuestions.length > 0 && (
              <div style={{ border: '1.5px solid #E5E7EB', borderRadius: 12, padding: '12px', maxHeight: 240, overflow: 'auto' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  ...
                  }} style={{ fontSize: '12px', color: 'var(--app-indigo)', background: 'none', border: 'none', cursor: 'pointer' }}>
                ...
                {aiGenQuestions.map((q, i) => (
                  <div key={i} onClick={() => {
                    ...
                    <div style={{ width: 18, height: 18, borderRadius: 4, border: '2px solid', borderColor: aiSelected.has(i) ? 'var(--app-indigo)' : 'var(--app-text-placeholder)', background: aiSelected.has(i) ? 'var(--app-indigo)' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '10px', fontWeight: 700, flexShrink: 0, marginTop: 1 }}>
                      {aiSelected.has(i) ? <CheckIcon size={10} color="#fff" /> : ''}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginBottom: '4px' }}>
                        <span style={{ padding: '1px 8px', borderRadius: 6, background: 'rgba(99,102,241,0.12)', color: 'var(--app-indigo)', fontSize: '11px', fontWeight: 600 }}>{QTYPE_LABELS[q.type] || q.type}</span>
            ...
            {aiGenQuestions.length > 0 ? (
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button onClick={handleAIGenerate} disabled={aiGenLoading}
                  style={{ padding: '12px 20px', background: 'var(--app-bg-page)', color: 'var(--app-indigo)', border: 'none', borderRadius: 12, fontSize: '14px', fontWeight: 500, cursor: 'pointer' }}>
                  重新生成
                </button>
              </div>
            ) : (
              <button onClick={handleAIGenerate} disabled={aiGenLoading}
                style={{ padding: '12px', background: 'var(--app-indigo)', color: '#fff', border: 'none', borderRadius: 12, fontSize: '15px', fontWeight: 500, cursor: aiGenLoading ? 'wait' : 'pointer', opacity: aiGenLoading ? 0.6 : 1 }}>
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.3)' }}>
      <div style={{ background: '#fff', borderRadius: 16, width: 660, maxWidth: '95vw', maxHeight: '90vh', overflow: 'auto', padding: '28px', boxShadow: '0 8px 32px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ display: 'flex', gap: '4px', marginBottom: '20px', background: 'var(--app-bg-page)', borderRadius: 10, padding: '3px' }}>
          ...
          <button onClick={() => setTab('ai')} style={{ flex: 1, padding: '8px', borderRadius: 8, border: 'none', fontSize: '13px', cursor: 'pointer', background: tab === 'ai' ? '#fff' : 'transparent', color: tab === 'ai' ? 'var(--app-indigo)' : 'var(--app-text-secondary)', fontWeight: 500 }}><BotIcon size={14} color="#6366F1" /> AI 出题</button>
        </div>
              ...
              <div>
                ...
                {generatedQuestions.length > 0 ? (
                  ...
                  <button onClick={handleAIGenerate} disabled={genLoading}
                    style={{ width: '100%', padding: '12px', background: 'var(--app-indigo)', color: '#fff', border: 'none', borderRadius: 10, fontSize: '14px', fontWeight: 500, cursor: genLoading ? 'not-allowed' : 'pointer', opacity: genLoading ? 0.6 : 1 }}>
            ...
            {aiMessages.length === 0 && !aiGeneratedQuestions.length ? (
              <div>
                ...
                <button onClick={async () => {
                  ...
                  style={{ width: '100%', padding: '12px', background: aiSending ? 'var(--app-text-muted)' : 'var(--app-indigo)', color: '#fff', border: 'none', borderRadius: 10, fontSize: '14px', fontWeight: 500, cursor: aiSending ? 'not-allowed' : 'pointer', opacity: aiSending ? 0.6 : 1 }}>
                ...
                <div style={{ maxHeight: 280, overflow: 'auto', marginBottom: '12px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {aiMessages.map((msg, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                      <div style={{ maxWidth: '80%', padding: '10px 14px', borderRadius: msg.role === 'user' ? '14px 14px 4px 14px' : '14px 14px 14px 4px', background: msg.role === 'user' ? 'var(--app-indigo)' : 'var(--app-bg-page)', color: msg.role === 'user' ? '#fff' : 'var(--app-text-heading)', fontSize: '13px', lineHeight: 1.6, wordBreak: 'break-word' }}>
                        {msg.role === 'user' ? (
                  ...
                  <div style={{ borderTop: '1px solid #E5E7EB', paddingTop: '12px', marginBottom: '12px' }}>
                    ...
                    {aiGeneratedQuestions.map((q, i) => (
                      <div key={i} onClick={() => {
                        ...
                        <div style={{ width: 18, height: 18, borderRadius: 4, border: '2px solid', borderColor: aiSelectedQuestions.has(i) ? 'var(--app-indigo)' : 'var(--app-text-placeholder)', background: aiSelectedQuestions.has(i) ? 'var(--app-indigo)' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '10px', fontWeight: 700, flexShrink: 0, marginTop: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      ...
                      <button onClick={() => setAiRegenExpanded(!aiRegenExpanded)}
                        style={{ padding: '12px 16px', background: aiRegenExpanded ? 'var(--app-brand-bg)' : 'var(--app-bg-page)', color: 'var(--app-indigo)', border: '1.5px solid', borderColor: aiRegenExpanded ? '#A5B4FC' : 'transparent', borderRadius: 10, fontSize: '13px', marginTop: '8px', cursor: 'pointer', fontWeight: 500 }}>
                        重新生成 <ChevronDownIcon size={12} color="#6366F1" />
                      </button>
                    </div>
                      ...
                      <div style={{ marginTop: '10px', padding: '14px', background: 'var(--app-bg-card-alt)', borderRadius: 12, border: '1.5px solid #E5E7EB' }}>
                        ...
                        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '10px' }}>
                          {AI_PRESET_FEEDBACK.map(fb => (
                            <button key={fb.label} onClick={() => handleAiRegen(fb.msg)}
                              style={{ padding: '6px 14px', background: '#fff', color: 'var(--app-indigo)', border: '1.5px solid #C7D2FE', borderRadius: 20, fontSize: '12px', cursor: 'pointer', fontWeight: 500 }}
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                          ...
                          <button onClick={() => aiRegenFeedback.trim() && handleAiRegen(aiRegenFeedback.trim())} disabled={aiSending || !aiRegenFeedback.trim()}
                            style={{ padding: '8px 16px', background: aiSending || !aiRegenFeedback.trim() ? 'var(--app-text-placeholder)' : 'var(--app-indigo)', color: '#fff', border: 'none', borderRadius: 8, fontSize: '13px', fontWeight: 500, cursor: aiSending || !aiRegenFeedback.trim() ? 'not-allowed' : 'pointer' }}>
                            {aiSending ? '生成中...' : '确认'}
                          </button>
                        </div>
                      </div>
                ...
                {aiGeneratedQuestions.length === 0 && (
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button onClick={handleAiSend} disabled={aiSending || !aiInput.trim()}
                      style={{ padding: '10px 20px', background: aiSending || !aiInput.trim() ? 'var(--app-text-muted)' : 'var(--app-indigo)', color: '#fff', border: 'none', borderRadius: 10, fontSize: '14px', fontWeight: 500, cursor: aiSending || !aiInput.trim() ? 'not-allowed' : 'pointer', opacity: aiSending || !aiInput.trim() ? 0.6 : 1 }}>
                      发送
                    </button>
                  </div>
                )}
