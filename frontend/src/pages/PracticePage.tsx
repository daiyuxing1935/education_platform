...
export default function PracticePage() {
  ...
  // Save practice state on unmount so it can be restored when returning
  useEffect(() => {
    return () => {
  ...
  useEffect(() => {
    ...
    // Restore saved practice state (e.g. returning from /path after viewing recommendations)
  ...
  const loadSessionQuestions = async (sid: string) => {
    ...
    try {
      ...
      setSessionId(sid)
      setReviewSelfGrades({})
      setAnswerMode('during')   // 回看模式: examMode=false → 显示正确/错误标记
      setReviewMode(true)       // 回看模式
      setTimeLimit(null)        // 回看模式禁用计时
  ...
  const handleEndPractice = async () => {
    ...
    if (answerMode === 'after' && currentQ) {
      questionCardRef.current?.submitSilent?.()
    }
    // 如果有未记录的答案，批量提交
    ...
    for (const ans of answers) {
      ...
      if (q && SUBJECTIVE_TYPES.includes(q.type)) {
        subjective.push({
          ...
          correctAnswer: q.answer?.correct_answer?.join(', ') || '(无标准答案)',
        })
      }
    }
      ...
      <div style={{ minHeight: '100vh', background: 'var(--app-bg-page)', padding: '12px 16px 100px' }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', marginBottom: '8px' }}>
            ...
            {isLastQuestion && isExamMode && !reviewMode ? (
              <span onClick={handleEndPractice}
                style={{
                  color: 'var(--app-danger)', cursor: 'pointer', fontSize: '13px',
                  padding: '8px 12px', fontWeight: 600, whiteSpace: 'nowrap',
                }}>
                提交 <ArrowRightIcon size={14} />
              </span>
            ) : (
              <span onClick={goNext}
                style={{
                  ...
                  <>
                    ...
                    <div style={{ marginBottom: '14px' }}>
                      ...
                      <div style={{ padding: '10px', background: 'rgba(16,185,129,0.06)', borderRadius: 10, fontSize: '13px', lineHeight: 1.6 }}>
                        <MarkdownRenderer content={currentQ.answer?.correct_answer?.join(', ') || '(无标准答案)'} />
                      </div>
                    </div>
                    {submitted ? (
        ...
        <ChatPanel visible={showChat} questionId={chatQId}
          question={(() => {
            ...
            return {
              ...
              answer: q.answer?.correct_answer?.join(', '),
