...
export default function BehaviorEventsPage() {
  ...
  const BEHAVIOR_TIME_OPTS: Intl.DateTimeFormatOptions = {
      year: 'numeric',
      month: '2-digit',
      day: '2-digit',
      hour: '2-digit',
  ...
  return (
    <div>
      ...
      {showRecordEvent && (
        <div className="card" style={{ marginBottom: '1.5rem', backgroundColor: 'var(--app-bg-card-alt)' }}>
      ...
      <div className="card">
        {activeTab === 'timeline' ? (
          <>
            ...
            {timeline.length === 0 ? (
              <div style={{ color: 'var(--app-text-secondary)', textAlign: 'center', padding: '2rem' }}>暂无时间线事件</div>
            ) : (
              <div style={{ display: 'grid', gap: '0.75rem' }}>
                {timeline.map((event: TimelineEvent) => (
                  <div key={event.event_id} style={{ padding: '0.75rem', backgroundColor: 'var(--app-bg-card-alt)', borderRadius: '0.375rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                      ...
                      <span style={{ fontSize: '0.75rem', color: 'var(--app-text-secondary)' }}>
                        {formatDateTime(event.timestamp, BEHAVIOR_TIME_OPTS)}
                      </span>
                    </div>
          ...
          <>
            ...
            {behaviorEvents.length === 0 ? (
              <div style={{ color: 'var(--app-text-secondary)', textAlign: 'center', padding: '2rem' }}>暂无行为事件</div>
            ) : (
              <div style={{ display: 'grid', gap: '0.75rem' }}>
