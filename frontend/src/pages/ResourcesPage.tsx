      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            {resource.resource_type === 'video' ? <VideoIcon size={18} /> : resource.resource_type === 'video_script' ? <VideoIcon size={18} /> : <BarChartIcon size={18} />}
            <h3 style={{ fontSize: '0.9375rem', fontWeight: 600, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {resource.title}
            </h3>
            {resource.resource_type === 'video' && (
              <span style={{ fontSize: '0.625rem', padding: '1px 5px', borderRadius: 8, background: '#fef3c7', color: '#d97706' }}>视频讲解</span>
            )}
            {resource.resource_type === 'video_script' && (
              <span style={{ fontSize: '0.625rem', padding: '1px 5px', borderRadius: 8, background: '#f0f0ff', color: '#7c3aed' }}>视频脚本</span>
            )}
          </div>
