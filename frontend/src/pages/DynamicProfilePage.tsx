    ...
    <div className="fade-in">
      ...
      <div style={{
        ...
        <div className="card" style={{ padding: 'var(--space-4)' }}>
          ...
          <RadarChart
            dimensions={[
              { name: '元认知校准', value: profile?.metacognitive_calibration ?? 0.5, color: 'var(--app-success)' },
              { name: '注意力特征', value: profile?.attention_feature ?? 0.5, color: 'var(--app-indigo)' },
              { name: '学习节奏', value: profile?.learning_rhythm?.scalar ?? 0.5, color: 'var(--app-warning)' },
            ]}
          />
          <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', marginTop: '8px' }}>
            {[
              { label: '元认知', value: profile?.metacognitive_calibration ?? 0.5, color: 'var(--app-success)' },
              { label: '注意力', value: profile?.attention_feature ?? 0.5, color: 'var(--app-indigo)' },
              { label: '学习节奏', value: profile?.learning_rhythm?.scalar ?? 0.5, color: 'var(--app-warning)' },
            ].map(d => (
        ...
        <div className="card" style={{ padding: 'var(--space-4)' }}>
          ...
          <TrendChart
            data={[
              { date: new Date(Date.now() - 6 * 86400000).toISOString().slice(0, 10), value: 12 },
              { date: new Date(Date.now() - 5 * 86400000).toISOString().slice(0, 10), value: 18 },
              { date: new Date(Date.now() - 4 * 86400000).toISOString().slice(0, 10), value: 22 },
              { date: new Date(Date.now() - 3 * 86400000).toISOString().slice(0, 10), value: 25 },
              { date: new Date(Date.now() - 2 * 86400000).toISOString().slice(0, 10), value: 30 },
              { date: new Date(Date.now() - 1 * 86400000).toISOString().slice(0, 10), value: 35 },
              { date: new Date().toISOString().slice(0, 10), value: 39 },
            ]}
            height={220}
            color="#6366F1"
          />
      ...
      <div className="card" style={{ padding: 'var(--space-4)', marginBottom: 'var(--space-8)' }}>
        ...
        <HeatmapCalendar
          data={[
            ...
            { date: new Date(Date.now() - 1 * 86400000).toISOString().slice(0, 10), count: 8 },
