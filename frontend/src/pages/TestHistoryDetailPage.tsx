      ...
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <div className="card-hover" style={{ background: '#fff', borderRadius: 16, padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.04)', marginBottom: '16px' }}>
          ...
          <div style={{ marginTop: '12px' }}>
            <button onClick={() => navigate(`/banks/${bankId}/practice?session_id=${sessionId}`)}
              style={{ padding: '8px 20px', background: 'var(--app-brand)', color: '#fff', border: 'none', borderRadius: 12, fontSize: '13px', fontWeight: 500, cursor: 'pointer' }}>
              回看此练习
            </button>
          </div>
        </div>
          ...
          {answers.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {answers.map((ans, i) => {
                ...
                const correctAnswer = (ans.question?.answer?.correct_answer || []).join(', ') || '见解析'
