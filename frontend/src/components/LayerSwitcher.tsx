...
export default function LayerSwitcher({
  ...
  return (
    <div style={{
      display: 'flex', background: 'var(--app-bg-page)', borderRadius: 10, padding: '4px', gap: '4px',
    }}>
      {layers.map(level => {
        ...
        return (
          <div key={level} onClick={() => onSelect(level)}
            ...
            <span style={{ fontSize: '13px', color: active ? '#fff' : 'var(--app-text-secondary)', fontWeight: 500 }}>
              {LAYER_CONFIG[level].label}
            </span>
            {recommended && (
              <span style={{ fontSize: '9px', background: 'var(--app-warning)', color: '#fff', padding: '1px 8px', borderRadius: 8 }}>
                推荐
              </span>
            )}
          </div>
