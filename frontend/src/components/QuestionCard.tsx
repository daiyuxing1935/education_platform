...
const QuestionCard = forwardRef<any, {
  ...
  // Initialize state from savedAnswer
  ...
  // Sync state when question changes (navigating back/forward)
  useEffect(() => {
    if (savedAnswer) {
      ...
      // In exam mode, keep submitted=false so inputs stay interactive
      if (!examMode) {
        setSubmitted(true)
  ...
  // Silent submit: saves answer to parent without changing visual state (for exam mode)
  const submitSilent = useCallback(() => {
    const answer = getCurrentAnswer()
    if (!answer.trim()) return
    const correct = checkAnswer(answer)
    // Don't change submitted/isCorrect state — keep form interactive
    onSubmit(answer, correct)
  }, [getCurrentAnswer, onSubmit, q.answer?.correct_answer])
  ...
  return (
    <div>
      <div className="card-hover" style={{
        ...
        {/* Type-specific input — in exam mode, always interactive */}
        {q.type === 'single_choice' && q.content.options && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
            {q.content.options.map(opt => (
              <div key={opt.key} onClick={() => !isDisabled && setSelected(opt.key)}
                style={{
        ...
        {q.type === 'multiple_choice' && q.content.options && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
            {q.content.options.map(opt => {
              ...
              return (
                <div key={opt.key} onClick={() => !isDisabled && toggleMulti(opt.key)}
                  style={{
                    display: 'flex', alignItems: 'center', padding: '14px 16px',
        ...
        {q.type === 'true_false' && (
          <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
            {['对', '错'].map(v => (
              <div key={v} onClick={() => !isDisabled && setSelected(v)}
                style={{
        ...
        {/* Submit / Result — hidden entirely in exam mode */}
        {!examMode && (
          submitted ? (
            <div>
              <div style={{
