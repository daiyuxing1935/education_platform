...
export default function MessageList({ messages, isLoading, enableThinking = false, onRunCode, onRollback, onEditDiagram }: MessageListProps) {
        ...
        >
          ...
          >
            ...
            {splitContentWithDiagrams(message.content).map((seg, segIdx) =>
              seg.type === 'text' ? (
                <div key={segIdx}>{renderMessageContent(seg.content)}</div>
              ) : (
                <DiagramImage key={`d-${segIdx}`} xml={seg.content} onEdit={onEditDiagram} />
              )
            )}
