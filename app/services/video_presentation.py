...
class VideoPresentationGenerator:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None, model: Optional[str] = None, tts_api_key: Optional[str] = None, unsplash_access_key: Optional[str] = None):
        ...
        # 视频生成强制使用 Qwen（千问），使用更长的超时时间
        self.timeout = settings.QWEN_TIMEOUT * 2  # 120s base, 180s for chapters
    ...
    def _resolve_config(self):
        ...
        Qwen 处理中文长文本质量更优，且支持更大上下文。
        用户传入了 API Key 则优先使用用户的，否则使用系统默认的 Qwen Key。
        """
        ...
        if qwen_key and not self.api_key:
            self.api_key = qwen_key
        if not self.api_key:
            self.api_key = settings.DEEPSEEK_API_KEY

        # 根据 API Key 判断 base_url
    ...
    async def _get_image_url_async(self, query: str, w: int = 1920, h: int = 1080) -> str:
        """通过 Unsplash API 获取图片 URL（完全异步，不阻塞事件循环）

        如果 Unsplash 不可用，直接返回 placehold.co 占位图 URL（纯字符串构造，无网络请求）。
        """
    ...
    async def _call_llm(self, system_prompt: str, user_prompt: str, max_tokens: Optional[int] = None, timeout_override: Optional[float] = None) -> Optional[str]:
        """调用 LLM（优先使用千问 Qwen API）

        关键设计：
        - 强制 Qwen 配置（视频生成场景中文长文本优先）
        - 异步 httpx 调用 + 指数退避重试
        - 超时后立即降级，不阻塞事件循环
        """
        if not self.api_key:
            logger.warning("视频生成：未配置 API Key")
            return None

        api_timeout = timeout_override or self.timeout
        temperature = self.temperature
        tok = max_tokens or self.max_tokens * 2  # 视频生成内容较多，默认放大
        ...
        try:
            ...
            logger.error(f"LLM 请求超时 (timeout={api_timeout}s)")
            return None
        except Exception as e:
    ...
    async def generate_chapters(self, script: str, outline: str, knowledge_points: List[str]) -> Optional[dict]:
        ...
        # 最多重试 2 次，超时提高到 180s（章节内容多，耗时较长）
        old_timeout = self.timeout
        self.timeout = 180.0
        try:
            for attempt in range(2):
                result = await self._call_llm(CHAPTERS_PROMPT, prompt, max_tokens=8192)
                if not result:
                    continue
    ...
    def build_presentation_html(self, title: str, chapters: list, output_path: str):
        """生成自包含 HTML 演示页面（同步版本，供非异步场景使用）"""
        return self._build_presentation_html_sync(title, chapters, output_path)

    async def build_presentation_html_async(self, title: str, chapters: list, output_path: str):
        """异步生成自包含 HTML 演示页面（预解析图片 URL，不阻塞事件循环）"""
        import asyncio

        topic_keywords = title.lower().replace(" ", ",")

        bg_images = []  # [(query, (w,h)), ...]
        image_steps = []  # [(query, (w,h)), ...]
        for ch in chapters:
            ch_keywords = ch.get("title", "").lower().replace(" ", ",")
            bg_images.append((ch_keywords or topic_keywords, (1920, 1080)))
            for s in ch.get("steps", []):
                img_query = s.get("image_query", "") or ch_keywords or topic_keywords
                bg_images.append((img_query, (1920, 1080)))
                if s.get("visual_type") == "image":
                    image_steps.append((img_query, (800, 450)))
                else:
                    image_steps.append(("", (800, 450)))

        # 异步并行解析所有图片 URL
        all_tasks = [self._get_image_url_async(q, w, h) for q, (w, h) in bg_images + image_steps]
        resolved = await asyncio.gather(*all_tasks) if all_tasks else []

        n_bg = len(bg_images)
        return self._build_presentation_html_sync(
            title, chapters, output_path,
            bg_urls=resolved[:n_bg],
            step_urls=resolved[n_bg:],
        )

    def _build_presentation_html_sync(
        self, title: str, chapters: list, output_path: str,
        bg_urls: list = None, step_urls: list = None,
    ):
        """实际构建 HTML 页面（纯字符串处理 + 文件写入，无网络请求）"""
        slides_html = ""
        durations = []
        audio_files = []
        bg_images_js = []
        ...
        for ch in chapters:
            ...
            # 章节背景图（使用预解析 URL 或 fallback）
            ch_bg = bg_urls[bg_idx] if bg_urls else self._fallback_bg(ch_keywords or topic_keywords)
            bg_idx += 1
            bg_images_js.append(ch_bg)
            ...
            for s in ch_steps:
                ...
                # 背景图（使用预解析 URL 或 fallback）
                step_bg = bg_urls[bg_idx] if bg_urls else self._fallback_bg(topic_keywords)
                bg_idx += 1
                bg_images_js.append(step_bg)
                ...
                # 配图（使用预解析 URL 或 fallback）
                if visual_type == "image":
                    img_url = step_urls[img_idx] if step_urls else self._fallback_bg(topic_keywords, 800, 450)
                    img_idx += 1
        ...
        bg_js = ", ".join(f'"{g}"' for g in bg_images_js)
    ...
    @staticmethod
    def _fallback_bg(query: str, w: int = 1920, h: int = 1080) -> str:
        """纯本地占位图 URL（无网络请求）"""
        return f"https://placehold.co/{w}x{h}/1a1a2e/ffffff?text={query.replace(' ', '+')}"
