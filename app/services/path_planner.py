...
GOAL_WEIGHT_MAP = {
    "学期提升": {"mastery_gap": 0.45, "importance": 0.20, "exam_frequency": 0.15, "cognitive_penalty": 0.20},
    "升学备考": {"mastery_gap": 0.30, "importance": 0.30, "exam_frequency": 0.30, "cognitive_penalty": 0.10},
    "考级考证": {"mastery_gap": 0.30, "importance": 0.35, "exam_frequency": 0.25, "cognitive_penalty": 0.10},
}
...
class PathPlanner:
    ...
    async def plan(self, student_id: str, goal_type: str = "") -> Dict[str, Any]:
        """执行路径规划算法，返回 ReactFlow 格式的 DAG 数据
        Args:
            student_id: 学生 ID
            goal_type: 目标类型（学期提升/升学备考/考级考证），用于动态调整权重
        """
        try:
            ...
            node_scores = self._compute_node_scores(sorted_names, all_scores, goal_type)
    ...
    def _compute_node_scores(self, node_names: List[str], weak_scores: Dict[str, float], goal_type: str = "") -> Dict[str, float]:
        """计算每个节点的加权优先级分数
        ...
        - 考级考证: 侧重重要度和考频，效率优先
        """
        weights = GOAL_WEIGHT_MAP.get(goal_type, DEFAULT_WEIGHTS)
        scores = {}

        for name in node_names:
            mastery = weak_scores.get(name, 0.0)  # 默认 0.0（无数据 = 未掌握）
    ...
    def _get_node_progress(self, node_names: List[str], weak_scores: Dict[str, float]) -> Dict[str, str]:
        ...
        - 0.3 <= score < 0.8: 进行中
        - score >= 0.8: 已完成（掌握良好）
        - score 不在 weak_scores 中: 未开始（无掌握度数据，默认为未学习）
        """
        progress = {}
        for name in node_names:
            score = weak_scores.get(name)
            if score is None:
                progress[name] = "not_started"  # 无数据 = 未开始，不能默认已完成
            elif score < 0.3:
